...
def check_semantic(ctx:Context, node):
	if node["nt"] == 'Program':
		for Decls_Defs in node["Decls_Defs"]:
			...
			assinatura = (Decls_Defs["nt"], Decls_Defs["type"], [arg["type"] for arg in Decls_Defs["arguments"]])
			for i in assinatura[2]:
				if i == "Void":
					assinatura[2].remove(i)
			if ctx.has_fun(name, assinatura):
				if assinatura[0] == "Definition":
					raise TypeError(f"Function {assinatura[0]} '{name}' is already defined in the context")
				else:
					raise TypeError(f"Function {assinatura[0]} '{name}' is already declared in the context")
		...
		for (i, (arg, par_t)) in enumerate(zip(node["arguments"], args_type)):
			...
			if counter > counter2 or counter < counter2:
				raise TypeError(f"'{name}' function call needs {counter2} arguments but got {counter}")
			if arg_t != par_t:
				index = i + 1
				raise TypeError(f"Argument number {index} in '{name}' function call was expecting {par_t} but got {arg_t}")
