...
def check_semantic(ctx:Context, node):
	if node["nt"] == 'Program':
		...
		if "array_create" not in ctx.using:
			raise TypeError("Array_create function not declared, 'using array_create;' is missing!")
		...
		expr = node["expr"]
		if expr["nt"] == "Lambda":
			assinatura = (expr["nt"], var_type, [arg["type"] for arg in expr["arguments"]], [arg["name"] for arg in expr["arguments"]])
			name = name + "_function"
			ctx.set_type(name, assinatura)
		...
		if "array_get" not in ctx.using:
			raise TypeError("Array_get function is not declared, 'using array_get;' is missing!")
		return node["name"]
		...
		if parameters["nt"] == "Lambda":
			if parameters["arguments"][0]["nt"] == "Void":
				return_type = check_semantic(ctx, parameters["expression"])
			else:
				return_type = parameters["arguments"][0]["type"]
			name = "$lambda$" + ctx.current_fun + "_function"
			assinatura = ("Definiton", return_type, [arg["type"] for arg in parameters["arguments"]], [arg["name"] for arg in parameters["arguments"]])
			ctx.set_type(name, assinatura)
		else:
			return_type = check_semantic(ctx, parameters)		
		...
		check_semantic(ctx, fun1)
		name = "$lambda$" + name
		fun2 = {'nt': 'fun_call', 'name': name, 'arguments': node["arguments2"]}
		return check_semantic(ctx, fun2)
		...
		for (i, (arg, par_t)) in enumerate(zip(node["arguments"], args_type)):
			...
			if counter > counter2 or counter < counter2:
				if "$lambda$" in naming:
					temp = naming.split("$lambda$")
					raise TypeError(f"Lambda from '{temp[1]}' function call needs {counter2} arguments but got {counter}")
				else:
			...
			if arg_t != par_t:
				...
				if "$lambda$" in naming:
					temp = naming.split("$lambda$")
					raise TypeError(f"Argument number {index} in Lambda from '{temp[1]}' function call was expecting {par_t} but got {arg_t}")
				else:
		...
		if "print" not in ctx.using:
			raise TypeError("Print function not declared, 'using print;' is missing!")
		...
		for arg in node["arguments"]:
			ctx.lambda_stack.append(arg)
		return check_semantic(ctx, expr)